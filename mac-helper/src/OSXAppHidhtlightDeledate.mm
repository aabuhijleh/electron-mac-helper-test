...
- (OSXAppHidhtlightDeledate *)initWithWindowId:(long)windowId {
  ...
  [center addObserver:self
             selector:@selector(onDeactivate:)
...
- (NSColor *)addBorderToBackgroundForWindow:(NSWindow *)window {
  ...
  NSRect bounds =
      NSMakeRect(0, 0, [window frame].size.width, [window frame].size.height);
  NSBezierPath *border = [NSBezierPath bezierPathWithRoundedRect:bounds
                                                         xRadius:10
                                                         yRadius:10];
...
- (void)updateWindow:(CFStringRef)notificationName {
  ...
  result = CFStringCompareWithOptions(
      ...
      CFRangeMake(0, CFStringGetLength(kAXWindowMiniaturizedNotification)),
  ...
  if (result == kCFCompareEqualTo) {
    ...
    for (NSMutableDictionary *entry in arr) {
      ...
      if (apid == pid) {
        ...
        NSArray *windowsArray = CFBridgingRelease(
        ...
        for (NSDictionary *dic in windowsArray) {
          if ([[dic objectForKey:@"kCGWindowOwnerPID"] intValue] == pid) {
            ...
            m_rect = CGRectMake(
                [[[dic objectForKey:@"kCGWindowBounds"] objectForKey:@"X"]
                    intValue],
                [[[dic objectForKey:@"kCGWindowBounds"] objectForKey:@"Y"]
                    intValue],
                [[[dic objectForKey:@"kCGWindowBounds"] objectForKey:@"Width"]
                    intValue],
                [[[dic objectForKey:@"kCGWindowBounds"] objectForKey:@"Height"]
                    intValue]);
            break;
          }
        }
        ...
        NSRect frame =
            NSMakeRect(m_rect.origin.x - 10,
                       ([NSScreen mainScreen].frame.size.height -
                        m_rect.origin.y - m_rect.size.height) -
                           10,
                       m_rect.size.width + 20, m_rect.size.height + 20);
        ...
        if (overlayWindow) {
          ...
          [overlayWindow
              setBackgroundColor:
                  [self addBorderToBackgroundForWindow:overlayWindow]];
  ...
  if (boolRef == NULL || CFBooleanGetValue(boolRef)) {
    ...
    *window = NULL;
    ...
    if (axError != kAXErrorSuccess) {
      NSLog(@"IN AppHidhtlightDeledate : windowWithProcessId error=%@",
            axError);
    }
  }
